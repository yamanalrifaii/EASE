# =============================================================
# eff_llm_optuna_fast.py
# EfficientNet-B0 + LSTM for EEG Spectrograms
#   • Optuna (TPE) HPO with real pruning & progress heartbeats
#   • Gemini LLM seeding + mid-run refinement (multi-calls)
#   • Robust retry with exponential backoff (429/DNS hiccups)
#   • Phase-2 uses new param names (no dynamic-space error)
#   • AMP + pinned memory + non_blocking H2D for speed
#   • Objective balances Accuracy & Sensitivity (equal weights)
...
# ====== Your modules ======
from architecture_eff import Net                        # expects (B, T, C, H, W) or your defined forward
from loader_spectrograms import load_patient_split      # ((X_train,y_train),(X_test,y_test)) or Dataset-based per your impl
...
torch.backends.cudnn.benchmark = True  # faster convs
...
def _gemini_call_with_retry(prompt: str, temperature: float = 0.25) -> str:
    """POST to Gemini with robust retry (handles 429 + transient network errors)."""
...
# leaner batches to keep things responsive
BATCH_CHOICES  = [16, 24, 32]
# search fast; we’ll polish the winner later
...
def oversample_pairs(X, y, factor=4):
    # if your loader returns torch tensors
...
for PATIENT in PATIENTS:
    ...
    # If you implemented the streaming Dataset, you can switch here.
    ...
    def fit_and_eval(cfg: Dict[str, Any], tag: str, trial: optuna.Trial = None) -> float:
        # Build loaders (enable pinned memory + non_blocking)
        ...
        try:
            ...
            for e in range(cfg["epochs"]):
                ...
                # Heartbeat + pruning signal
                if (e+1) % 10 == 0 or e == 0:
                    print(f"[{tag}] epoch {e+1}/{cfg['epochs']} loss={loss.item():.4f}")
                if trial is not None:
                    proxy = -float(loss.item())  # lower loss = better
    ...
    for lr0 in [3e-5, 7.5e-5, 3e-4]:
        study.enqueue_trial({"lr": lr0, "batch": 32, "epochs": 60})
...
"""
            ...
            # enqueue a safe fallback if we already refined
    ...
    # Save best (works for r1 or r2 param names)
    ...
    # --- Optional polish: retrain best once with longer epochs ---
