...
# Main loader
# =============================================================
def load_patient_split(patient_id, spec_dir, test_size=0.3, augment=True, seq_len=3, use_cuda=True):
    """
    ...
    # Move to GPU if available and specified
    ...
    print("train labels:", dict(zip(*np.unique(y_train.cpu().numpy(), return_counts=True))))
    print("test  labels:", dict(zip(*np.unique(y_test.cpu().numpy(), return_counts=True))))
    print("Sample batch mean/std:", float(X_train.mean()), float(X_train.std()))

    # Return DataLoader for easy batching
