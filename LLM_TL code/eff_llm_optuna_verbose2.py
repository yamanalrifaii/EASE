...
if __name__ == "__main__":
    ...
    mp.set_start_method('spawn')  # Ensures proper process spawning on Windows
...
BATCH_CHOICES = [64, 80, 96]
EPOCH_CHOICES  = [60, 90, 120]
...
def scan_best_threshold(y_true, y_prob):
    ...
    for th in np.linspace(0.05, 0.9, 171):
        ...
        val = 0.35*acc + 0.65*sens  # equal weight accuracy + sensitivity
        if val > best_val:
            best_val, best_th, best_pack = val, th, (acc, sens, spec, f1, f1w)
    return best_th, best_pack, best_val

def oversample_pairs(X, y, min_pos_ratio=0.15, max_factor=3):
    ...
    # target: at least 15% seizure windows, but never oversample >3×
    needed = int(len(neg) * min_pos_ratio)
    factor = min(max(1, needed // len(pos)), max_factor)

    pos_rep = np.tile(pos, factor)
    idx = np.concatenate([neg, pos_rep])
    np.random.shuffle(idx)

    # Add slight augmentation for positives
    if factor > 1:
            noise = np.random.normal(0, 0.02, X[pos_rep].shape).astype(np.float32)
            X_aug = X[pos_rep] + noise
...
def run_patient(PATIENT: str):
    ...
    def fit_and_eval(cfg: Dict[str, Any], tag: str) -> float:
        ...
        Xb, yb = oversample_pairs(X_train, y_train, max_factor=4)
        ...
        print(f"[{PATIENT}] Acc={acc*100:.2f}% Sens={sens*100:.2f}% Spec={spec*100:.2f}% "
              f"F1={f1*100:.2f}% F1w={f1w*100:.2f}% | Obj(0.35A+0.65S)={obj:.6f}")
    ...
    for lr0 in [3e-5, 7.5e-5, 3e-4]:
        study.enqueue_trial({"lr": lr0, "batch": 64, "epochs": 90})
